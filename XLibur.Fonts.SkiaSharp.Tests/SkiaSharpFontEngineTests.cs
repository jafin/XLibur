...
public class SkiaSharpFontEngineTests
{
    ...
    public async Task FontEngine_CanSaveAndReloadWorkbook()
    {
        ...
        await Assert.That(value).IsEqualTo("Saved with SkiaSharp");
    }
    ...
    {
        await Assert.That(() => new SkiaSharpFontEngine(null!)).Throws<ArgumentException>();
    }

    [Test]
    public async Task Constructor_ThrowsOnWhitespaceFallbackFont()
    {
        await Assert.That(() => new SkiaSharpFontEngine("   ")).Throws<ArgumentException>();
    }

    [Test]
    public async Task CreateOnlyWithFonts_ThrowsOnNullStream()
    {
        await Assert.That(() => SkiaSharpFontEngine.CreateOnlyWithFonts(null!)).Throws<ArgumentNullException>();
    }
