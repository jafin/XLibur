...
{
    [Test]
    public async Task InsertColumnBeforeCol1_PreservesMultiColumnDvSqrefsThroughSave()
    {
        ...
        {
            await Assert.That(rule1Out.sqref).IsEqualTo("B2:C2").Because("Multi-column dv straddling the insert point lost its sqref during save.");
            await Assert.That(rule2Out.sqref).IsEqualTo("E2:F2").Because("Multi-column dv right of the insert point lost its sqref during save.");
        }
    }

    [Test]
    public async Task InsertColumnBeforeCol1_ManyAdjacentDvsAllSurviveWithCorrectSqrefs()
    {
        ...
        await Assert.That(empties).IsEmpty().Because($"Saved file has {empties.Count} dv(s) with empty sqref — Excel will reject "
        ...
        using (Assert.Multiple())
        {
            await Assert.That(FindSqref(dvs, "Straddle")).IsEqualTo("B2:C2");
            await Assert.That(FindSqref(dvs, "DataRange")).IsEqualTo("K3:L100");
            for (var col = 3; col <= 9; col++)
            {
                var expectedCol = XLHelper.GetColumnLetterFromNumber(col + 1);
                await Assert.That(FindSqref(dvs, $"Cell{col}")).IsEqualTo($"{expectedCol}2:{expectedCol}2").Because($"Cell{col} dv should have shifted from column {col} to {col + 1}.");
            }

            var withFormulaOut = dvs.Single(x => x.title == "WithFormula");
            await Assert.That(withFormulaOut.sqref).IsEqualTo("N3:N50").Because("Formula-bearing dv lost or mis-shifted its sqref.");
            await Assert.That(withFormulaOut.f1).IsEqualTo("3");
