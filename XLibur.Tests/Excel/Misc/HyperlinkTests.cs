...
{
    [Test]
    public async Task TestHyperlinks()
    {
        ...
        await Assert.That(linkCell1.GetHyperlink().InternalAddress).IsEqualTo("Sheet2!A1");

        var linkRange1 = ws1.Cell("A2");
        linkRange1.Value = "Link to IXLRangeBase";
        linkRange1.SetHyperlink(new XLHyperlink(targetRange));
        await Assert.That(linkRange1.GetHyperlink().InternalAddress).IsEqualTo("Sheet2!A1:B1");
    }
}
